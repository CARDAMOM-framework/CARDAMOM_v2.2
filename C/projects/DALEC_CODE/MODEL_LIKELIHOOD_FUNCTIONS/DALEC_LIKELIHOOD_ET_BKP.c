...
/* For log_et_obs*/
double thresh=D.et_obs_threshold;
...
for (n=0;n<D.net;n++){dn=D.etpts[n];tot_exp+=pow(log(D.M_FLUXES[dn*D.nofluxes+28]/D.ET[dn])/log(max(1,thresh/D.ET[dn])*D.et_obs_unc),2);}}
else{
...
for (n=0;n<12;n++){dn=D.etpts[n+m*12];tot_exp+=pow(log(D.M_FLUXES[dn*D.nofluxes+28]/D.ET[dn]*am/mam)/log(max(1,thresh/D.ET[dn])*D.et_obs_unc),2);}
