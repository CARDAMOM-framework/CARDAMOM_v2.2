...
int EDC2_101(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD)
{
...
printf("log(EQF) = %f\n",log(EQF));
printf("etol = %f\n",etol);
printf("fabs(log(Rs)) = %2.2f\n",fabs(log(Rs)));
printf("fabs(Rs-Rm) = %2.2f\n",fabs(Rs-Rm));
...
for (n=0;n<nopools;n++){if ((EDC==1 || DIAG==1) & ((MPOOLS[n])>parmax[n+11])){EDC=0;EDCD->PASSFAIL[35-1]=0;}}
