...
int EDC2_813(double const *pars, DATA DATA, struct EDCDIAGNOSTIC *EDCD)
{
...
/* EDC2_SUMMARY
6. Cfol:Croo
7-13. Pool trajectory EDCs
14. Ensuring that wilting point is at or below the mean H2O pool
35.
*/
...
if (((EDC==1 & DIAG==0) || DIAG==1 || (EDC==1 & DIAG==2 & EDCD->SWITCH[7-1+n]==1)) & (pars[25]>MPOOLS[6])){EDC=ipow(0,EDCD->SWITCH[14-1]);EDCD->PASSFAIL[14-1]=0;}
...
for (n=0;n<nopools;n++){if ((EDC==1 || DIAG==1) & ((MPOOLS[n])>parmax[pidx[n]])){EDC=0;EDCD->PASSFAIL[35-1]=0;}}
